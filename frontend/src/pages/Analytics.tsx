...
interface IndividualSummary { stake: number; unique_selections: number; settled_selections: number; eligible_predictions?: number; settled_predictions?: number; excluded_missing_odds?: number; excluded_post_kickoff_information?: number; excluded_post_kickoff_odds?: number; overall: IndividualRow; by_date: IndividualRow[]; by_month: IndividualRow[]; by_year: IndividualRow[]; by_market: IndividualRow[]; by_competition: IndividualRow[] }
...
function AutoMarketResearch({ data, setData }: { data: IndividualSummary | null; setData: (value: IndividualSummary | null) => void }) {
  ...
  const excluded = (data.excluded_missing_odds ?? 0) + (data.excluded_post_kickoff_information ?? 0) + (data.excluded_post_kickoff_odds ?? 0)
  return <><div className="journal-note"><strong>Automatic all-market research</strong><span>Every latest prediction with a valid pre-kickoff information timestamp and odds snapshot is evaluated. This does not create bets or alter the paper ledger.</span></div><div className="research-kpis"><MetricCard label="Eligible predictions" value={data.eligible_predictions?.toString() ?? '—'} /><MetricCard label="Settled" value={data.settled_predictions?.toString() ?? '—'} /><MetricCard label="Net P&amp;L" value={fmtPnl(data.overall.pnl)} tone={data.overall.pnl >= 0 ? 'positive' : 'negative'} /><MetricCard label="ROI" value={pct(data.overall.roi)} tone={data.overall.roi >= 0 ? 'positive' : 'negative'} /></div>{excluded > 0 && <div className="sample-warning">Evidence exclusions: {excluded} rows omitted ({data.excluded_post_kickoff_information ?? 0} post-kickoff predictions, {data.excluded_post_kickoff_odds ?? 0} post-kickoff quotes, {data.excluded_missing_odds ?? 0} missing/invalid odds).</div>}<SortableIndividualTable title="By day" rows={data.by_date} /><SortableIndividualTable title="By month" rows={data.by_month} /><SortableIndividualTable title="By year" rows={data.by_year} /><SortableIndividualTable title="By market" rows={data.by_market} market /><SortableIndividualTable title="By league" rows={data.by_competition} /><ClvPanel /></>
}
